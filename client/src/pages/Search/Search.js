...
const SearchPage = () => {
	...
	useEffect(() => {
		getSearchDataWithPages(selected, searchData, currentPage)
			.then(data => {
                setResults(data);
                window.scrollTo(0, 0);
            })
				...
				<>
					...
					<CardList>
						{results.results?.map((item) => (
							<Link key={item.id} to={`/${selected}/${item.id}/details`}>
								<Card
									poster={
										selected === 'person' ? item.profile_path : item.poster_path
									}
									title={selected === 'movie' ? item.title : item.name}
								/>
							</Link>
						))}
					</CardList>
					<Pagination
						...
						lastPage={results.total_pages > 500 ? 500 : results.total_pages}
					/>
					<BackToTop />
				</>
			);
	...
	return (
		<div>
			<h1 className={classes.heading}>
				Search by typing a word or phrase in the search box at the top of this
				page.
			</h1>
